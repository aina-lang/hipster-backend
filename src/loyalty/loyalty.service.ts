...
export class LoyaltyService {
    ...
    async getLoyaltyStatus(clientId: number): Promise<LoyaltyStatus> {
        ...
        if (projectCount >= LOYALTY_RULES[LoyaltyTier.GOLD].minProjects) tier = LoyaltyTier.GOLD;
        else if (projectCount >= LOYALTY_RULES[LoyaltyTier.SILVER].minProjects) tier = LoyaltyTier.SILVER;
        else if (projectCount >= LOYALTY_RULES[LoyaltyTier.BRONZE].minProjects) tier = LoyaltyTier.BRONZE;
        ...
        if (tier === LoyaltyTier.STANDARD) {
            ...
            projectsToNextTier = LOYALTY_RULES[LoyaltyTier.BRONZE].minProjects - projectCount;
            progress = (projectCount / LOYALTY_RULES[LoyaltyTier.BRONZE].minProjects) * 100;
        } else if (tier === LoyaltyTier.BRONZE) {
            nextTier = LoyaltyTier.SILVER;
            projectsToNextTier = LOYALTY_RULES[LoyaltyTier.SILVER].minProjects - projectCount;
            const range = LOYALTY_RULES[LoyaltyTier.SILVER].minProjects - LOYALTY_RULES[LoyaltyTier.BRONZE].minProjects;
            const currentInTier = projectCount - LOYALTY_RULES[LoyaltyTier.BRONZE].minProjects;
            progress = (currentInTier / range) * 100;
        } else if (tier === LoyaltyTier.SILVER) {
            nextTier = LoyaltyTier.GOLD;
            projectsToNextTier = LOYALTY_RULES[LoyaltyTier.GOLD].minProjects - projectCount;
            const range = LOYALTY_RULES[LoyaltyTier.GOLD].minProjects - LOYALTY_RULES[LoyaltyTier.SILVER].minProjects;
            const currentInTier = projectCount - LOYALTY_RULES[LoyaltyTier.SILVER].minProjects;
            progress = (currentInTier / range) * 100;
        }
    ...
    async getClientLoyaltyDetail(clientId: number) {
        ...
        for (const project of completedProjects) {
            cumulativeCount++;
            let tier = LoyaltyTier.STANDARD;
            if (cumulativeCount >= LOYALTY_RULES[LoyaltyTier.GOLD].minProjects) tier = LoyaltyTier.GOLD;
            else if (cumulativeCount >= LOYALTY_RULES[LoyaltyTier.SILVER].minProjects) tier = LoyaltyTier.SILVER;
            else if (cumulativeCount >= LOYALTY_RULES[LoyaltyTier.BRONZE].minProjects) tier = LoyaltyTier.BRONZE;
        ...
        return {
            ...
            projectsInProgress: client.projects.filter((p) => p.status === ProjectStatus.IN_PROGRESS).length,
        };
    }
    ...
    async updateClientLoyaltyOnProjectCompletion(projectId: number): Promise<{
        ...
        const totalBudget = completedProjects.reduce((sum, p) => sum + Number(p.budget || 0), 0);
